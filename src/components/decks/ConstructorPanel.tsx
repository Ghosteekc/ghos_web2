...
import type { CardDisplayMode, ConstructorData, Deck, DeckCard } from "@/types";
...
function toDeck(entry: ConstructorData["decks"][number]): Deck {
  return {
    id: entry.id,
    name: entry.name,
    cards: entry.cards,
  ...
  } as Deck & { synergy_score?: number; synergy_notes?: string[] };
}
...
export function ConstructorPanel({ renderDeckCard }: ConstructorPanelProps) {
  ...
  const buildDecks = useCallback(async (current: (SlotPick)[]) => {
    ...
    setError(null);
    try {
      const data = await api.buildConstructorDecks(
        picks.map((p) => ({ name: p.name, slot: p.slot })),
      );
      setDecks(data.decks.map(toDeck));
    } catch (e) {
      setDecks([]);
      setError(e instanceof ApiError ? e.message : "Не удалось собрать колоды");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => void buildDecks(slots), 400);
