    ...
    <div className="grid grid-cols-2 gap-2">
      {items.map((item, index) => (
        <Card
          ...
          delay={index * 0.04}
          hover={false}
          className="!p-3 text-center aspect-[4/3] flex flex-col items-center justify-center gap-1 min-w-0"
        >
          ...
          <p className="text-[10px] text-cr-muted leading-tight px-1">{item.label}</p>
        </Card>
      ))}
    </div>
  );
